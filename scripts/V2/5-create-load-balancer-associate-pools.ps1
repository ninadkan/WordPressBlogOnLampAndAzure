...
if ($loadBalancer)
{
    ...
    if (-not $healthProbe)
    {
        $healthProbe = Add-AzLoadBalancerProbeConfig -Name $healthProbeName -LoadBalancer $loadBalancer  -Protocol Http -Port 443 -IntervalInSeconds 20 -ProbeCount 2 -RequestPath "/"
        Set-AzLoadBalancer -LoadBalancer $loadBalancer
    }
    ...
    {
       $loadBalancerRule = Add-AzLoadBalancerRuleConfig -LoadBalancer $loadBalancer -Name $loadBalancerRuleName -FrontendPort 443 `
